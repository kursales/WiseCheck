...
public class CreateActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.edt_Bday:
            new DatePickerDialog(CreateActivity.this, dateD, myCalendar
    ...
    private void downloadFile(String path) {
        ...
        call.enqueue(new Callback<ResponseBody>() {
            ...
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                checkFile = writeResponceBodyToDisk(response.body());
            }
            @Override
            public void onFailure(Call<ResponseBody> call, Throwable t) {
                Toast.makeText(CreateActivity.this, "Отсутствует интернет-соединение", Toast.LENGTH_SHORT).show();
                Log.d("stud","ua ua ua");
    ...
    private boolean writeResponceBodyToDisk(ResponseBody body) {
        try {
            ...
            try {
               ...
               Toast.makeText(CreateActivity.this,"Неверный URL", Toast.LENGTH_SHORT).show();
                return false;
            } finally {
    ...
    public  void update(){
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                listEmployee.get(listEmployee.size()-1).setName(edFio.getText().toString());
                listEmployee.get(listEmployee.size()-1).setBirthday(edDate.getText().toString());
                listEmployee.get(listEmployee.size()-1).setProfession(spinner.getSelectedItem().toString());
                listEmployee.get(listEmployee.size()-1).setPathToFile(path);
                listEmployee.get(listEmployee.size()-1).setUrl(edUri.getText().toString());
                employeeDao.insert(listEmployee.get(listEmployee.size()-1));
